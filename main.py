...
from utils.calculate_weights import calculate_weight_labels
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.use_balanced_weights:
            classes_weights_path = os.path.join(dataset_root_dir(args.dataset), args.dataset + '_classes_weights.npy')
            if os.path.isfile(classes_weights_path):
                weight = np.load(classes_weights_path)
            else:
                weight = calculate_weight_labels(args.dataset, self.train_loader, self.nclass)
    ...
    def validation(self, epoch):
        ...
        mean_iou = self.evaluator.mean_iou()
        fw_iou = self.evaluator.frequency_weighted_iou()
        self.writer.add_scalar('val/total_loss_epoch', test_loss, epoch)
        self.writer.add_scalar('val/mIoU', mean_iou, epoch)
        self.writer.add_scalar('val/Acc', acc, epoch)
        self.writer.add_scalar('val/Acc_class', acc_class, epoch)
        self.writer.add_scalar('val/fwIoU', fw_iou, epoch)
        print('Validation:')
        print('[Epoch: %d, numImages: %5d]' % (epoch, i * self.args.batch_size + image.data.shape[0]))
        print("Acc:{}, Acc_class:{}, mIoU:{}, fwIoU: {}".format(acc, acc_class, mean_iou, fw_iou))
...
def main():
    parser = argparse.ArgumentParser(description="PyTorch Deeplab-v3+ Training")
